...
# El juego original no trae niveles de dificultad separados.
# La IA base usa bastante azar para moverse, saltar y reaccionar.
# Este flag reduce ese azar sólo para jugadores bot, sin tocar físicas.
...
class Igrac1(pygame.sprite.Sprite):
    ...
    def update_move(self, stipke, koji, nogica1, sutnuo1, nogica2, sutnuo2, skok_zvuk, FRIC, kontrole, platforme, sirina, loptaa, poss, gravitacija, broj_lopti, gol_sirina, lista_igraca):
        ...
        if(self.bot and EXPO_TOP1_HARD_AI): br1=150
        else: br1=random.randrange(int(self.radius*2), 150)
        ...
        if(nogica1.delay>0 and sutnuo1):
            if(self.bot and EXPO_TOP1_HARD_AI): ok1=True
            else:
                br5=random.randrange(0,4)
                if(br5>0):ok1=True
            
        if(self.bot and EXPO_TOP1_HARD_AI): br2=max(2, round(self.radius/10))
        else: br2=random.randrange(round(self.radius/6), round(self.radius+20))

        v3=vec(lopta.pos)
        v4=vec(self.pos)
        dist2=v3.distance_to(v4)
        if(self.bot and EXPO_TOP1_HARD_AI): br3=150
        else: br3=random.randrange(int(self.radius*2), 150)
        ...
        if(nogica2.delay>0 and sutnuo2):
            if(self.bot and EXPO_TOP1_HARD_AI): ok2=True
            else:
                br5=random.randrange(0,4)
                if(br5>0):ok2=True
            
        if(self.bot and EXPO_TOP1_HARD_AI): br4=max(2, round(self.radius/10))
        else: br4=random.randrange(round(self.radius/6), round(self.radius+20))

        if(self.bot and EXPO_TOP1_HARD_AI): br6=max(2, round(self.radius/8))
        else: br6=random.randrange(round(self.radius/5), round(self.radius+10))
        nazad1,nazad2=False,False

        if(self.bot and EXPO_TOP1_HARD_AI): br7=max(2, round(self.radius/3))
        else: br7=random.randrange(round(self.radius/2), round(self.radius+15))
